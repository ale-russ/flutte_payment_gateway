...
const MOMO_API_BASE_URL = isProduction
  ? process.env.MOMO_API_BASE_URL_PRODUCTION
  : process.env.MOMO_API_BASE_URL_SANDBOX;
const X_TARGET_ENVIRONMENT = isProduction
  ? process.env.X_TARGET_ENVIRONMENT_PRODUCTION
  : process.env.X_TARGET_ENVIRONMENT_SANDBOX;
...
async function createBearerToken(subscriptionKey, xReferenceId, apiKey) {
  ...
  const authHeader = `Basic ${Buffer.from(`${xReferenceId}:${apiKey}`).toString(
    "base64"
  )}`;
  try {
    const response = await axios.post(
...
async function pollPaymentStatus(
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.log(
        `Payment not completed yet, retrying in ${
          delay / 1000
        } seconds (Attempt ${i + 1}/${retries})`
      );
    } catch (error) {
...
async function processPayment(amount, phone, reference) {
  ...
  try {
    ...
    if (isProduction) {
      ...
      console.log(
        "Production Mode: Using API_USER_PRODUCTION and API_KEY_PRODUCTION from .env"
      );
    ...
    const tokenSanitized = await createBearerToken(
      subscriptionKey,
      xRefIdForToken,
      apiKey
    );
    ...
    const paymentStatusResponse = await pollPaymentStatus(
      subscriptionKey,
      tokenSanitized,
      xReferenceId
    );
    ...
    console.log(
      ...
      paymentStatusResponse.status.replace(/["']/g, "")
    );
